    ...
    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 bg-slate-50 border-b border-slate-100">
        <button type="button" onClick={prevMonth}
          className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-200 text-slate-600 text-lg">‹</button>
...
export default function BookingSection({ cardId, bookingSettings }: Props) {
  ...
  async function submit() {
    if (!name.trim() || !phone.trim() || !date || !time) {
      setError('이름, 연락처, 날짜, 시간을 모두 입력해 주세요')
      return
    }
    setError('')
    setLoading(true)
  ...
  return (
    <section className="px-5 py-6 border-t border-slate-100">
      <h2 className="text-base font-bold text-slate-800 mb-4">📅 예약하기</h2>

      {done ? (
        <div className="text-center py-10 bg-blue-50 rounded-2xl">
          <p className="text-4xl mb-3">✅</p>
          <p className="font-bold text-slate-800 text-lg">예약 신청 완료!</p>
          <p className="text-sm text-slate-500 mt-1">확인 후 연락드리겠습니다.</p>
          <button
            type="button"
            onClick={() => { setDone(false); setName(''); setPhone(''); setDate(''); setTime(''); setNote(''); setGuests(1) }}
            className="mt-5 px-6 py-2.5 bg-blue-500 text-white rounded-2xl font-semibold text-sm"
          >
            다시 예약하기
          </button>
        </div>
